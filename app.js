...
async function loadHealth(){
  try{
    ...
    $('#liveText').textContent=stockReady
...
async function loadCrypto(){try{const d=await api('/api/crypto?limit=100&meme_limit=70');state.crypto=d.data.map(x=>({...x,type:'crypto'}));renderMarket();renderWatch();connectBinance()}catch(e){$('#assetList').innerHTML=`<div class="error-box">${esc(e.message)}</div>`}}
...
async function loadDetail(){const a=state.selected;if(!a)return;try{const baseUrl=a.type==='crypto'?`/api/crypto-analyze?symbol=${encodeURIComponent(a.symbol)}&tf=${state.timeframe}`:`/api/analyze?symbol=${encodeURIComponent(a.symbol)}&tf=${state.timeframe}`;const cached=state.mtfCache.get(`${a.type}:${a.symbol}`),mtfP=cached&&Date.now()-cached.ts<60000?Promise.resolve(cached.data):api(`/api/multi-analyze?type=${a.type==='crypto'?'crypto':'market'}&symbol=${encodeURIComponent(a.symbol)}`).then(d=>(state.mtfCache.set(`${a.type}:${a.symbol}`,{ts:Date.now(),data:d}),d));const [d,mtf,order,back,ctx]=await Promise.all([api(baseUrl),mtfP,a.type==='crypto'?api(`/api/orderflow?symbol=${encodeURIComponent(a.symbol)}`).catch(()=>null):Promise.resolve(null),api(`/api/backtest?type=${a.type==='crypto'?'crypto':'market'}&symbol=${encodeURIComponent(a.symbol)}&tf=${['15m','1h','4h','1d'].includes(state.timeframe)?state.timeframe:'4h'}`).catch(()=>null),api(`/api/context?type=${encodeURIComponent(a.type)}&symbol=${encodeURIComponent(a.symbol)}`).catch(()=>null)]);state.analysis={d,mtf,order,back,ctx};renderDetail(a,d,mtf,order,back,ctx)}catch(e){$('#detailContent').innerHTML=`<div class="error-box">${esc(e.message)}</div>`}}
